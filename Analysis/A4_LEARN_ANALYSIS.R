...
path <- '~/Desktop/sexcog/Figures/A4_LEARN/'
load('~/Desktop/sexcog/Preprocessing/A4LEARN2025-06-15.RData')

## Demographics ----
A4_LEARN_cross_sectional <- A4_LEARN %>%
  arrange(ID, pacc_time) %>%
  group_by(ID) %>%
  slice_tail()

demographics <- A4_LEARN_cross_sectional %>%
  ungroup() %>%
...
extract_pacc_stats(A4_LEARN)
extract_pacc_stats(A4_LEARN, FALSE)


## PCA on tau ----
A4_LEARN <- A4_LEARN %>% ungroup()
tau.roi <- dplyr::select(A4_LEARN, "bl_Amyg","bl_Para", "bl_ITG", "bl_EC", "bl_Fus", "bl_MTG")
...
## PC 1 captures 82% of variance

## Main effect of sex -----
A4_LEARN_cross_sectional$SEX <- relevel(A4_LEARN_cross_sectional$SEX, ref = "M")
lm_sex_Amyg <- lm(data = A4_LEARN_cross_sectional, bl_Amyg ~ SEX +bl_tau_age)
summary(lm_sex_Amyg)
tab_model(lm_sex_Amyg, show.std = TRUE)

lm_sex_Para <- lm(data = A4_LEARN_cross_sectional, bl_Para ~ SEX +bl_tau_age)
summary(lm_sex_Para)
tab_model(lm_sex_Para, show.std = TRUE)

lm_sex_EC <- lm(data = A4_LEARN_cross_sectional, bl_EC ~ SEX +bl_tau_age)
summary(lm_sex_EC)
tab_model(lm_sex_EC, show.std = TRUE)

lm_sex_Fus <- lm(data = A4_LEARN_cross_sectional, bl_Fus ~ SEX +bl_tau_age)
summary(lm_sex_Fus)
tab_model(lm_sex_Fus, show.std = TRUE)

lm_sex_ITG <- lm(data = A4_LEARN_cross_sectional, bl_ITG ~ SEX +bl_tau_age)
summary(lm_sex_ITG)
tab_model(lm_sex_ITG, show.std = TRUE)

lm_sex_MTG <- lm(data = A4_LEARN_cross_sectional, bl_MTG ~ SEX +bl_tau_age)
summary(lm_sex_MTG)
...
lme_amyg <- lme(PACC~ SEX*bl_Amyg*pacc_time + 
                         ...
                         YrsEd*pacc_time +
                         QSVERSION + Cumulative_Dose_Scaled, 
...
plot_model(lme_amyg, type = "pred", terms = c("pacc_time [all]", "SEX", "bl_Amyg"), condition = c(Cumulative_Dose_Scaled = 0)) +
  ylim(-9, 6) +
  theme_bw() + 
...
png(outfile, width = 17, height =7, units = "cm", res = 500)
plot_model(lme_para, type = "pred", terms = c("pacc_time [all]", "SEX", "bl_Para"), condition = c(Cumulative_Dose_Scaled = 0)) +
  ylim(-9, 6) +
  theme_bw() + 
  scale_x_continuous(breaks = seq(-2, 10, by = 2)) +
  theme(
...
png(outfile, width = 17, height =7, units = "cm", res = 500)
plot_model(lme_EC, type = "pred", terms = c("pacc_time [all]", "SEX", "bl_EC"), condition = c(Cumulative_Dose_Scaled = 0)) +
  ylim(-9, 6) +
  theme_bw() + 
  scale_x_continuous(breaks = seq(-2, 10, by = 2)) +
  theme(
...
png(outfile, width = 17, height =7, units = "cm", res = 500)
plot_model(lme_Fus, type = "pred", terms = c("pacc_time [all]", "SEX", "bl_Fus"), condition = c(Cumulative_Dose_Scaled = 0)) +
  ylim(-9, 6) +
  theme_bw() + 
  scale_x_continuous(breaks = seq(-2, 10, by = 2.0)) + 
  theme(
    panel.grid = element_blank(),
    plot.title = element_text(hjust = 0.5, face = "bold.italic", size = 17),
    panel.border = element_blank(),
...
png(outfile, width = 17, height =7, units = "cm", res = 500)
plot_model(lme_ITG, type = "pred", terms = c("pacc_time [all]", "SEX", "bl_ITG"), condition = c(Cumulative_Dose_Scaled = 0)) +
  ylim(-9, 6) +
  theme_bw() + 
  scale_x_continuous(breaks = seq(-2, 10, by = 2.0)) + 
  theme(
...
png(outfile, width = 17, height =7, units = "cm", res = 500)
plot_model(lme_MTG, type = "pred", terms = c("pacc_time [all]", "SEX", "bl_MTG"), condition = c(Cumulative_Dose_Scaled = 0)) +
  ylim(-9, 6) +
  theme_bw() + 
  scale_x_continuous(breaks = seq(-2, 10, by = 2.0)) + 
  theme(
  ...
  labs(title = "Middle temporal tau", x = "PACC time from baseline tau-PET (years)", y = "PACC score", color = "SEX")
dev.off()

## Model 1: Sensitivity analysis ----
### Adjusting for centiloid x time
lme_amyg_amyloid <- lme(PACC~ SEX*bl_Amyg*pacc_time + 
                          bl_tau_age*pacc_time +
                          YrsEd*pacc_time +
                          QSVERSION + Cumulative_Dose_Scaled + centiloid*pacc_time, 
                        na.action = na.exclude, 
                        random = (~pacc_time | ID), 
                        data =A4_LEARN, 
                        control = lmeControl(opt = 'optim'),
...
### Adjusting for APOE x time
lme_amyg_apoe <- lme(PACC~ SEX*bl_Amyg*pacc_time + 
                       bl_tau_age*pacc_time +
                       YrsEd*pacc_time +
                       QSVERSION + Cumulative_Dose_Scaled + APOE*pacc_time, 
                     na.action = na.exclude, 
                     random = (~pacc_time | ID), 
                     data =A4_LEARN, 
                     control = lmeControl(opt = 'optim'),
...
## Create PACC slope
A4_PACC <- A4_LEARN
pure_pacc <- lme(PACC ~ pacc_time, random = list(ID = pdDiag(~ pacc_time)), data = A4_PACC)
lme_slope <- coef(pure_pacc)$pacc_time
A4_LEARN_cross_sectional$lme_pacc_slope <- lme_slope
A4_LEARN_cross_sectional$SEX <- relevel(A4_LEARN_cross_sectional$SEX, ref = "M")
A4_LEARN_cross_sectional$SEX<-as.numeric(A4_LEARN_cross_sectional$SEX)

## Johnson Neyman
mod<-lm(lme_pacc_slope ~ SEX*bl_Amyg, data=A4_LEARN_cross_sectional, na.action = na.exclude); summary(mod)
...
mod<-lm(lme_pacc_slope ~ SEX*bl_Para, data=A4_LEARN_cross_sectional, na.action = na.exclude); summary(mod)
...
mod<-lm(lme_pacc_slope ~ SEX*bl_EC, data=A4_LEARN_cross_sectional, na.action = na.exclude); summary(mod)
jn_results_EC <- johnson_neyman(model = mod, pred =SEX,  modx = bl_EC)
outfile <- file.path(path, "jn_EC.png")
png(outfile, width = 16, height =14, units = "cm", res = 500)
jn_results_EC$plot +
  labs(x = "Baseline Entorhinal SUVR", 
       y = "Slope of SEX") +
  theme(legend.position = "bottom") 
dev.off() 

## Model 1: sex x regional tau-PET x time (prospective only) ----
A4_LEARN_prospective <- A4_LEARN %>%
  filter(pacc_time >= 0)
mean(A4_LEARN_prospective$pacc_time)
sd(A4_LEARN_prospective$pacc_time)
A4_LEARN_prospective$SEX <- relevel(A4_LEARN_prospective$SEX, ref = "M")
